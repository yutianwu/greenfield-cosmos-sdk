...
func initFixture(t testing.TB) *fixture {
	keys := storetypes.NewKVStoreKeys(
		authtypes.StoreKey, authzkeeper.StoreKey, banktypes.StoreKey, distrtypes.StoreKey, stakingtypes.StoreKey,
	)
	...
	newCtx := sdk.NewContext(cms, types.Header{}, true, nil, logger)
	...
	authzKeeper := authzkeeper.NewKeeper(keys[authzkeeper.StoreKey], cdc, nil, accountKeeper)

	stakingKeeper := stakingkeeper.NewKeeper(cdc, keys[stakingtypes.StoreKey], accountKeeper, authzKeeper, bankKeeper, authority.String())
...
func TestMsgWithdrawDelegatorReward(t *testing.T) {
	...
	validator, err := stakingtypes.NewSimpleValidator(f.valAddr, PKS[0], stakingtypes.Description{})
