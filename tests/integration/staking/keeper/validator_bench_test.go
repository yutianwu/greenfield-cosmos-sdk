...
func BenchmarkGetValidatorDelegations(b *testing.B) {
	...
	b.ResetTimer()
	for n := 0; n < b.N; n++ {
		updateValidatorDelegations(ctx, app, valAddrs[0], sdk.AccAddress("val"))
	}
}
...
func BenchmarkGetValidatorDelegationsLegacy(b *testing.B) {
	...
	b.ResetTimer()
	for n := 0; n < b.N; n++ {
		updateValidatorDelegationsLegacy(ctx, app, valAddrs[0], sdk.AccAddress("val"))
	}
}

func updateValidatorDelegationsLegacy(ctx sdk.Context, app *simapp.SimApp, existingValAddr, newValAddr sdk.AccAddress) {
...
func updateValidatorDelegations(ctx sdk.Context, app *simapp.SimApp, existingValAddr, newValAddr sdk.AccAddress) {
	storeKey := app.GetKey(types.StoreKey)
	cdc, k := app.AppCodec(), app.StakingKeeper
